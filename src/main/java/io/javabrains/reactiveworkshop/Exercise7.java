...
public class Exercise7 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        ReactiveSources.intNumbersFlux()
                .filter(e -> e > 20)
                .defaultIfEmpty(-1) // It gets executed when this flux gets the completion event from the filter flux
                .subscribe(System.out::println);

        // Switch ints from intNumbersFlux to the right user from userFlux
        ReactiveSources.intNumbersFlux()
                .flatMap(number -> ReactiveSources.userFlux().filter(user -> user.getId() == number)).subscribe(System.out::println);
